...
class ConvNetRP(nn.Module):
    def __init__(self, d, c_in=1, c1=50, c2=100):
        ...
        self.conv1 = nn.Conv2dRP(in_channels=c_in, out_channels=c1, kernel_size=3, padding=0, d=d)
        self.relu1 = nn.ReLU(inplace=True)
        self.mp1 = nn.MaxPool2d(2)
        
        self.conv2 = nn.Conv2dRP(in_channels=c1, out_channels=c2, kernel_size=3, padding=0, d=d)
        self.relu2 = nn.ReLU(inplace=True)
        self.mp2 = nn.MaxPool2d(2)
        
        self.av = nn.AvgPool2d(5)
        self.lin1 = nn.LinearRP(c2, 10, d=d)
